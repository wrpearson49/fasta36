...
my $get_annots_refseq = $dbh->prepare(qq(select acc, ex_p_start as start, ex_p_end as end, ex_num as ix, chrom, g_start, g_end from seqdb_demo2.ref_exons where acc=? order by ix));
...
sub show_annots {
  ...
  unless ($use_acc) {
    ...
    if ($sdb =~ m/ref/) {
      $get_annots_sql = $get_annots_refseq;
    } else {
...
C<ann_exons_all.pl> extracts exon location information from msyql
databases (uniprot for Uniprot proteins, seqdb_demo2 for refseq) built
from EBI/proteins API data (Uniprot) or Refseq GFF data (refseq).
...
C<ann_exons_all.pl --gen_coord 'sp|P09488|GSTM1_HUMAN'>also provides genomic coordinates:
...
C<ann_exons_all.pl> is designed to be used by the B<FASTA> programs
with the C<-V \!ann_exons_all.pl> option, or by the
C<annot_blast_btop.pl> script.  It can also be used with the
lav2plt.pl program with the C<--xA "\!ann_exons_all.pl --lav"> or
C<--yA "\!ann_exons_all.pl --lav"> options.
